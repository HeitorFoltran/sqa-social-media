...
public class PostService {
  ...
  public Map<String, Object> getPosts(Integer limit, Integer skip, Long userId) {
    try {
      ...
      for (JsonNode postNode : rootNode.get("posts")) {
        posts.add(mapPost(postNode, likedPostIds));
      }
  ...
  public Map<String, Object> getLikedPosts(Long userId, Integer limit, Integer skip) {
    try {
      ...
      for (Long postId : paginatedIds) {
        ...
        Map<String, Object> post = mapPost(postNode, new HashSet<>(likedPostIds));
        posts.add(post);
      }
  ...
  private Map<String, Object> mapPost(JsonNode postNode, Set<Long> likedPostIds) {
    ...
    if (reactions != null) {
      if (reactions.has("likes"))    likes    = reactions.get("likes").asInt();
      if (reactions.has("dislikes")) dislikes = reactions.get("dislikes").asInt();
    }
